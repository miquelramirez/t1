 ...
 * @note @e Please email me if you encounter any bugs in my code so I may fix
 *       them.
 *
...
class MinMaxHeap // Root is on Max level
{
     ...
     *        @c zindex.
     **/
    inline unsigned int parent(unsigned int zindex) const
    {
        return (zindex - 1) / 2;
    }
     ...
     *        @c zindex.
     **/
    inline unsigned int leftChild(unsigned int zindex) const
    {
        return 2 * zindex + 1;
    }
     ...
     *        @c zindex.
     **/
    inline unsigned int rightChild(unsigned int zindex) const
    {
        return 2 * zindex + 2;
    }
     ...
     *        @e min-level.
     **/
    inline bool isOnMinLevel(unsigned int zindex) const
    {
     ...
     *        @e max-level.
     **/
    inline bool isOnMaxLevel(unsigned int zindex) const
    {
    ...
    void trickleDown_(unsigned int zindex)
    {
        ...
        if (zindex >= heap_.size())
            ERROR(std::invalid_argument("Element specified by zindex does not exist"));
        ...
        if (smallestNode - left > 1) trickleDown_<MaxLevel>(smallestNode);
    }
    ...
    unsigned int findMinIndex() const
    {
        ...
        switch (heap_.size())
        {
            case 0:
                ...
                ERROR(std::underflow_error("No min element exists because there are no elements in the heap."));
            case 1:
    ...
    void deleteElement(unsigned int zindex)
    {
        ...
        if (zindex >= (unsigned int)heap_.size())
            ERROR(std::underflow_error("Cannot delete specified element from the heap because it does not exist."));
...
public:
    ...
    MinMaxHeap(Container zcontainer, Compare zcompare = Compare()) : heap_(zcontainer), compare_(zcompare) { }
    ...
    /**
     * @brief Convenience function that calls findMin().
     **/
    T top()
    {
        return findMax();
    ...
    /**
     * @brief Convenience function that calls popMin().
     **/
    T pop()
    {
        return popMax();
    ...
    /**
     * @brief Keeps the heap at size N
     **/
        void maintain_size( const unsigned sizeN )
        {
                int diff = ( heap_.size() - sizeN );
                if ( diff > 0) remove_worst( diff );
        }

    /**
     * @brief Removes the n worst elements
     **/
        void remove_worst( const unsigned n )
        {
                NFF::Planner_Stats&		stats = NFF::Planner_Stats::instance();
                for ( unsigned i = 0; i < n; i++ )
                {
                        stats.substract_node_size( popMin()->size() );
                }
        }
    ...
    const T & findMax() const
    {
        ...
        if (empty())
            ERROR(std::underflow_error("No max element exists because there are no elements in the heap."));
