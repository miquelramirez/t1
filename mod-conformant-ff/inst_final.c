...

{
  ...
  case OR:
	...
	tmp->preconds[0] = lindex[lp][adr];
      ...
      tmp->effects[0].adds_nondet = ( Bool * ) calloc( 1, sizeof( Bool ) );
      tmp->effects[0].adds_nondet[0] = FALSE;
...

{
  ...
  goneof_or_corr = ( int * ) calloc( gnum_full_oneof_initial, sizeof( int ) );
  ...
  for ( a = gactions; a; a = a->next ) {
	...
	continue;
      ...
      if ( (e->num_conditions + a->num_preconds) > 0 )
  ...
  for ( i = 0; i < gnum_full_oneof_initial; i++ ) {
    ...
    goneof_or_corr[gnum_oneofs++] = gnum_initial_or;
    gnum_initial_or++;
  ...
  if ( gcmd_line.display_info == 121 ) {
    ...
    printf("\n\n----------------------INITIAL ONEOFS:-----------------------------");
    printf("\nTotal Oneofs: %d",gnum_full_oneof_initial);

    for ( i = 0; i < gnum_full_oneof_initial; i++ ) {
      printf("\nONEOF: ");
      int or_corr = goneof_or_corr[i];
      for ( j = 0; j < ginitial_or_length[or_corr]; j++ ) {
	print_ft_name( ginitial_or[or_corr][j] );
	printf(" ");
      }
    }
