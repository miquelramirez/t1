...
namespace FF
{
  ...
  inline void 		get_initial_unknown( std::vector<unsigned>& unkn_atoms )
  {
    unkn_atoms.resize( ginitial_state.num_U );
    for ( int i = 0; i < ginitial_state.num_U; i++ )
      unkn_atoms[i] = ginitial_state.U[i];
  }
  
  
  inline void 		get_initial_ors ( std::vector<std::vector<unsigned> >& clauses )
  {
    clauses.resize( gnum_initial_or );
    for ( int i = 0; i < gnum_initial_or; i++ ) {
      clauses[i].resize( ginitial_or_length[i] );
      for ( int j = 0; j < ginitial_or_length[i]; j++ ) 
	clauses[i][j] = ginitial_or[i][j] ;
    }
  }
  

  inline void 		get_initial_oneofs ( std::vector<unsigned>& oneofs_corr )
  {
	  oneofs_corr.resize( gnum_full_oneof_initial );

	  // Copying the correspondent or into the oneof 
	  for ( int i = 0; i < gnum_full_oneof_initial; i++ ) {
		  oneofs_corr[i] = goneof_or_corr[i];  
	  }
  }
  

  inline bool             get_ft_sign ( int i )
  {
    return !(gnot_fact[ grelevant_facts[i].predicate ]);
  }
  
  /* fluent's negation; the resp ft number if it is translated,
   * otherwise -1
   */ 
  inline int             get_ft_negated ( int i )
  {
    return gft_conn[i].negation ;
  }

  inline float		get_op_metric_cost( int index )
  {
    return 1; /*gef_conn[index].cost + gtt;*/
  }
  ...
  {
    goal_atoms.resize( ggoal_state.num_F );
    for ( int i = 0; i < ggoal_state.num_F; i++ )
	{
		goal_atoms[i] = ggoal_state.F[i];
	}
  }
