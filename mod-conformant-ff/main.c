...
/* List of facts that are negative */
...

{
  ...
  /* ...and for vplan file 
   */
  char vplan_file[MAX_LENGTH] = "";


  int i;
  Bool found_plan, valid_plan;
  ...
  memset(gcmd_line.vplan_file_name, 0, MAX_LENGTH);
  ...
  /* ...and vplan_file */
  sprintf(vplan_file, "%s%s", gcmd_line.path, gcmd_line.vplan_file_name);
  ...
  if (validation) {
    times( &vstart );    
    read_input_plan(get_vplan_file_name());

    valid_plan = FALSE;
    
    if ( gcmd_line.display_info >= 1 ) 
      printf("\n---- Start validating plan file");
 
    /* Should create ehc_node with path 0 */
    EhcNode *node0 = new_BfsNode();
    copy_source_to_dest( &(node0->S) , &ginitial_state);
    node0->op = -1;/* intial state has op -1 */

    valid_plan =  validate( node0 );
    
    if ( gcmd_line.display_info >= 1 ) 
      printf("\n---- End validating plan file\n");

    times( &vend );
    gverif_time =( float ) ( ( vend.tms_utime - vstart.tms_utime + \
			       vend.tms_stime - vstart.tms_stime  ) / 100.0 );
    output_planner_info();
    printf("\n\n");
...
char *get_vplan_file_name( )
{
  return gcmd_line.vplan_file_name;
}
...

{
  ...
  if (validation) 
    printf( "\n            %7.2f seconds validating the plan", gverif_time );
  ...
  printf( "\n            %7.2f seconds total time (+ %7.2f secs for CNF memory allocation)", 
	  gtempl_time + greach_time + grelev_time + gconn_time + genc_time + gsearch_time + gsat_time + geval_time + gr_sat_time + grp_sat_time + gr_cnf_time + gr_enc_time + gcnf_time + grs_time + gss_time + grs_sat_time + gverif_time, gmem_time);
...

{
  ...
  printf("-v <str>    plan file name (validation mode)\n\n");
...

{
  ...
  while ( --argc && ++argv ) {
	...
	case 'f':
	  ...
	  break;
	case 'v':
	  strncpy( gcmd_line.vplan_file_name, *argv, MAX_LENGTH );
	  validation = TRUE;
	  break;
	case 'i':
